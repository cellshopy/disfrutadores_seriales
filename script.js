// ===== CONFIG =====
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyEUMff0DqhP6HN_ryKPcO3XRngaCptaNqHcueYWyBa7zkCASYW2r_iwZhhWlMRLfDA/exec";

// Links a secciones (ajustables si quieren subcategorías específicas)
const redirectMap = {
...
function getTopTwo(){
  ...
  finalPrimary = p1[0];
  finalScore = p1[1];
  finalSecondary = (p2 && (p1[1]-p2[1] <= 1)) ? p2[0] : null; // híbrido si está cerca
}
...
function buildInterestChecks(primary, secondary){
  ...
  // sugerimos checkeado el primary (+ secondary si existe)
...
function buildResultCopy(primary, secondary){
  ...
  // texto híbrido simple y compartible
  const combos = {
    "TECH+BEAUTY": "Tenés estética en alta definición: te gusta que funcione perfecto y que se vea/sienta mejor todavía.",
    "BEAUTY+TECH": "Tenés estética en alta definición: te gusta que funcione perfecto y que se vea/sienta mejor todavía.",
    "GOURMET+HOME": "Sos placer bien servido: rituales en casa y algo rico para convertir cualquier día en ocasión.",
    "HOME+GOURMET": "Sos placer bien servido: rituales en casa y algo rico para convertir cualquier día en ocasión.",
    "OUTDOOR+HOBBY": "Sos adrenalina con obsesión: salís a buscar historias y después te quedás horas perfeccionando tu pasión.",
    "HOBBY+OUTDOOR": "Sos adrenalina con obsesión: salís a buscar historias y después te quedás horas perfeccionando tu pasión.",
  };
...
btnNext.addEventListener("click", () => {
  ...
  if (idx === questions.length - 1) {
    // terminar -> calcular y mostrar lead screen
    computeScores();
    getTopTwo();
...
leadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (out.status !== "success") throw new Error(out.message || "Error desconocido");

    // pintar resultado
    const baseTitle = archetypes[finalPrimary].title;
    const finalTitle = finalSecondary
      ? `${baseTitle} (híbrido con ${finalSecondary})`
      : baseTitle;

    resultTitle.textContent = finalTitle;
    resultText.textContent = buildResultCopy(finalPrimary, finalSecondary);

    // link a sección correspondiente + utms para tracking
